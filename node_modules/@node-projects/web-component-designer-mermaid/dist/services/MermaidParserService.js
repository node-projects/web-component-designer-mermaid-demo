...
import { getMermaidDocumentDiagramType, getMermaidDocumentFrontmatter, getMermaidDocumentTitle, mermaidDiagramTypeAttribute, mermaidFlowchartDirectionAttribute, mermaidFrontmatterAttribute, mermaidTitleAttribute } from "./MermaidDocumentPropertiesService.js";
import { encodeWaypoints, routeBetweenBounds } from "./mermaidGeometry.js";
...
function getDiagramKind(document) {
...
async function parseFlowchart(html, serviceContainer, instanceServiceContainer) {
...
function writeFlowchart(textWriter, designItems, updatePositions, rootDesignItem) {
    ...
    const explicitlyWrittenNodeIds = new Set();
    const writeNodeOrSubgraph = (designItem, depth) => {
        const element = designItem.element;
        if (element.nodeName === "MERMAID-SUBGRAPH") {
            const subgraphElement = element;
...
function writeFlowchartEdgeLine(textWriter, designItem, element, nodeItemsById, explicitlyWrittenNodeIds, updatePositions) {
...
function getFlowchartNodeIdsUsedByEdges(designItems) {
    const ids = new Set();
    for (const designItem of designItems) {
        const element = designItem.element;
...
function getFlowchartNodeItemsById(designItems) {
...
function getAllFlowchartItems(designItems) {
    const result = [];
    const append = (item) => {
        result.push(item);
...
function shouldWriteFlowchartNode(element, nodeIdsUsedByEdges) {
    const id = element.getAttribute("node-id");
...
async function parseSequenceDiagram(html, serviceContainer, instanceServiceContainer) {
    ...
    const designItems = [];
    const participantBounds = new Map();
    let index = 0;
...
function writeSequenceDiagram(textWriter, designItems, updatePositions, rootDesignItem) {
    writeMermaidFrontmatter(textWriter, designItems, rootDesignItem);
...
async function parseMindmapDiagram(html, serviceContainer, instanceServiceContainer) {
    const document = readMermaidDocument(html);
...
function writeMindmapDiagram(textWriter, designItems, updatePositions, rootDesignItem) {
    writeMermaidFrontmatter(textWriter, designItems, rootDesignItem);
    textWriter.writeLine("mindmap");
    const writeItems = (items, depth) => {
...
async function parseRequirementDiagram(html, serviceContainer, instanceServiceContainer) {
    const document = readMermaidDocument(html);
    const lines = document.lines;
    const nodes = new Map();
    const relationships = [];
...
function writeRequirementDiagram(textWriter, designItems, updatePositions, rootDesignItem) {
    const rootDirection = rootDesignItem?.getAttribute(mermaidFlowchartDirectionAttribute);
...
function parseSequenceParticipant(line, sourceStart) {
...
function parseSequenceMessage(line, sourceStart) {
...
function ensureSequenceParticipant(participants, id) {
    if (!participants.has(id))
        participants.set(id, { id, label: id, participantType: "participant", implicit: true });
}
function getSequenceMessageType(connector) {
...
function parseFlowchartDirection(line) {
    const cursor = new SourceCursor(line);
    const keyword = cursor.readIdentifier();
...
function parseDirectionStatement(line) {
    const cursor = new SourceCursor(line);
    const keyword = cursor.readIdentifier();
...
function parseSubgraphStart(line, sourceStart, parentId) {
    const cursor = new SourceCursor(line);
    const keyword = cursor.readIdentifier();
...
function parseFlowchartDirective(line, sourceStart) {
    const cursor = new SourceCursor(line);
    const keyword = cursor.readIdentifier();
    if (!keyword)
        return null;
...
function parseBinaryStatement(line, operators) {
...
function parseRequirementBlockStart(line) {
    const cursor = new SourceCursor(line);
...
function parseRequirementBlock(blockStart, lines, sourceStart, sourceLength) {
...
function parseRequirementRelationship(line, sourceStart) {
...
function parseRequirementRelationshipForward(line, sourceStart) {
    const cursor = new SourceCursor(line);
...
function parseRequirementRelationshipReverse(line, sourceStart) {
    const cursor = new SourceCursor(line);
    const to = cursor.readIdentifier();
    if (!to)
        return null;
...
function parseFlowchartEdgeStatement(line) {
...
function parseExpandedShape(value) {
...
function parseDelimitedShapeValue(value) {
    const candidates = [
...
function parseObjectLiteralProperties(content) {
...
function findOperator(line, operators) {
    let depth = 0;
    let quote = "";
...
async function getMermaidFlowchartData(code) {
    try {
...
async function getMermaidSequenceData(code) {
    try {
        const mermaid = await import("mermaid");
        mermaid.default.initialize({ startOnLoad: false, securityLevel: "loose" });
        const diagram = await mermaid.default.mermaidAPI.getDiagramFromText(code);
        ...
        if (!actors || !mermaidMessages)
            return null;
        const participants = Array.from(actors.entries()).map(([id, actor]) => ({
            id,
            label: actor.description ?? actor.name ?? id,
            participantType: actor.type ?? "participant",
        }));
        const messages = mermaidMessages
...
function mapMermaidEdgeType(edge) {
...
function mapMermaidSequenceConnector(type) {
    switch (type) {
        case 1:
            return "-->>";
        case 3:
...
function normalizeMermaidText(value) {
    if (value == null)
        return "";
...
function stripMarkdownString(value) {
    let result = value;
...
function parseNode(line, sourceStart) {
    const cursor = new SourceCursor(line);
    const id = cursor.readIdentifier();
    if (!id)
        return null;
...
function parseMindmapNode(line, index, parentId, sourceStart, sourceLength, indent) {
...
function parseEdge(line, sourceStart) {
...
function parseShapeValue(id, value) {
    if (!value)
...
function mapExpandedShape(shape) {
    switch (shape) {
        case "rect":
        case "proc":
        case "process":
...
function mapMindmapShape(shape) {
    switch (shape) {
        case "rectangle":
            return "square";
        case "round":
...
function sanitizeMindmapId(label, index) {
    let result = "";
...
function sanitizeDirectiveId(label) {
    let result = "";
    for (const char of label) {
        if (isIdentifierPart(char))
            result += char;
...
function getDefaultMindmapNodeSize(shape) {
    switch (shape) {
        case "circle":
        case "bang":
...
function getDefaultRequirementNodeSize(node) {
...
function getFallbackRequirementNodePosition(index, total, direction) {
...
function getFallbackFlowchartNodePosition(index, count, direction) {
    const xStep = 180;
    const yStep = 110;
    switch (direction) {
        case "LR":
...
function readMermaidDocument(text) {
...
function readFrontmatterWithoutTitle(lines) {
...
function getDiagramDirection(designItems) {
    const rootDesignItem = getRootDesignItem(designItems);
    const rootDirection = rootDesignItem?.getAttribute(mermaidFlowchartDirectionAttribute);
...
function inferDiagramDirection(designItems) {
...
function isFlowchartDirection(value) {
    return value === "TB" || value === "TD" || value === "BT" || value === "RL" || value === "LR";
}
function isRequirementDirection(value) {
    return value === "TB" || value === "BT" || value === "RL" || value === "LR";
}
function setMermaidDocumentAttributes(instanceServiceContainer, diagramType, direction, title, frontmatter) {
...
function writeMermaidFrontmatter(textWriter, designItems, rootDesignItem) {
...
function getWritableDocumentDiagramType(designItems) {
...
function writeDesignItemLine(textWriter, designItem, line, updatePositions) {
    const start = textWriter.position;
...
function indentEmbeddedNewlines(line) {
...
async function getMermaidLayout(code, nodes) {
...
async function getMermaidRequirementLayout(code, nodes) {
    if (typeof document === "undefined" || typeof DOMParser === "undefined")
        return null;
    try {
        const mermaid = await import("mermaid");
...
async function getMermaidMindmapLayout(code, nodes) {
    if (typeof document === "undefined" || typeof DOMParser === "undefined")
        return null;
    try {
        const mermaid = await import("mermaid");
...
function parseTranslateFromElementTree(element) {
    let x = 0;
    let y = 0;
    let current = element;
    while (current) {
...
function getFlowchartNodeIdFromDomId(domId) {
...
function getRequirementNodeIdFromSvgId(svgId, renderId, nodes) {
    if (!svgId)
        return "";
...
function getClusterIdFromDomId(domId, renderId) {
    if (!domId)
        return "";
...
function isRequirementRelationshipType(value) {
...
function getSvgNodeSize(nodeElement) {
...
function getMindmapWidgetSize(svgSize, shape) {
