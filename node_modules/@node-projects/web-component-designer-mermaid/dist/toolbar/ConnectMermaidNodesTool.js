import { DesignItem, EventNames, InsertAction, OverlayLayer } from "@node-projects/web-component-designer";
import { encodeWaypoints, getAnchor, routeBetweenBounds, routeBetweenPoints, pathDataFromWaypoints } from "../services/mermaidGeometry.js";
...
function findNode(event, currentElement) {
...
export class ConnectMermaidNodesTool {
    cursor = "crosshair";
    _captureElement;
    _pointerId;
    _previewPath;
    ...
    keyboardEventHandler(designerCanvas, event) {
        if (event.key === "Escape")
            this._cleanup(designerCanvas, true);
    }
    _begin(designerCanvas, event, currentElement) {
        ...
        this._captureElement = event.target;
    ...
    _update(designerCanvas, event, currentElement) {
    ...
    _finish(designerCanvas, event, currentElement) {
        ...
        const edge = document.createElement(this._getRelationshipTag(designerCanvas));
        ...
        designerCanvas.instanceServiceContainer.undoService.execute(new InsertAction(designerCanvas.rootDesignItem, designerCanvas.rootDesignItem.childCount, designItem));
        this._cleanup(designerCanvas, true);
        event.preventDefault();
        event.stopPropagation();
    }
    _cleanup(designerCanvas, finished) {
    ...
    _getRelationshipTag(designerCanvas) {
